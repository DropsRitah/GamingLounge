...
app = Flask(__name__)
app.secret_key = 'gaming_lounge_graduation_2024_secure'

# --- ABSOLUTE DATABASE PATH (Crucial for Render) ---
basedir = os.path.abspath(os.path.dirname(__file__))
db_path = os.path.join(basedir, 'gaming_lounge.db')
...
def admin_dashboard():
    ...
    today = date.today()
    # Fixed query to prevent crashes if no sessions exist
    completed = GameSession.query.filter(db.func.date(GameSession.end_time) == today, GameSession.is_active == False).all()
    total_sales = sum(s.total_cost for s in completed) if completed else 0
    
    return render_template('admin_dashboard.html', 
                           stations=Station.query.all() or [], 
                           games=Game.query.all() or [], 
                           total_sales=round(total_sales, 2),
                           reports=completed or [])
...
def stop_session(id):
    ...
    if gs:
        ...
        gs.total_cost = round(max(duration * (game.rate_per_hour if game else 200), 10.0), 2)
